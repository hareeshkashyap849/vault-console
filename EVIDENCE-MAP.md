...
| **the two sources fail independently** | **Structural assertions + the real service** | `src/app/vault/page.tsx`'s **four independent `useQuery` calls** (`live` / `status` / `candles` / `priceSeries`), each with its own `error` and its own panel branch<br>`test/api.test.ts` → 6 failure-classification assertions<br>`tools/browser-assert.mjs` → `the four panels rendered`, `no service-failure panel is showing`<br>**The stop-the-service re-runs (measured 2026-09-16, and no longer applicable to the shipped artifact)**: `tools/capture-scenarios.mjs` started a console instance whose `VAULT_RPC` points at a dead port (3123) and one whose `VAULT_API` does (3122), saved both pages and judged them — scenarios 9 and 10, both exit 0. Those two variables configure a **dev-server proxy**; a static export has neither the proxy nor those variables, so the pair is retained as the record of what was measured and is not evidence for the published pages. What the published page does instead is carry the `no-route` kind (§1's last row and `src/lib/serviceFailure.ts`) | the classification logic is right; when both are available all four panels are there and no failure copy is shown; **and** when one upstream is genuinely refused the other panel still renders its own figures | **cannot** prove this holds for a service that dies **mid-request** as opposed to being refused at connect time — the injection method is a refused connection, which is the same thing from the caller's side but not for a timeout or a half-read response. Those two are covered by doubles only (`test/api.test.ts`), which is recorded in §3. Nor does any of it cover **one query failing while another in the same page still renders on a static host**: that is the four-query structure's own claim, asserted structurally in the source and not measured against the export |
...
| ~~**`dynamic = 'force-dynamic'` takes effect**~~ | **RETIRED 2026-09-17, and the property it proved is now UNPROVEN** — see `docs/STATIC-EXPORT-MIGRATION.md` | The mechanism is gone: the pages are client components in a static export and no longer export `dynamic = 'force-dynamic'`. The recorded measurement (`12639 → 12648`, `97 → 2293 blocks`, no restart, `docs/evidence/scenario-11-freshness.txt`) proved that the **server** re-read per request. A static host has no server to re-read; the page issues its own reads on every load | the replacement -- `staleTime: 0`, `refetchOnWindowFocus`, and the `no-store` row above -- is a unit test plus a structural argument (no server, no cache layer, JSON-RPC POSTs are not cacheable). **That is not the same evidence**, and it has not been re-measured against the published site | **Re-measure before relying on it**: load the published page twice, capture the RPC requests each load issues, and record the pair. Until that exists, treat "nothing is cached" as unproven for the shipped artifact |
...
| ~~**`dynamic = 'force-dynamic'` takes effect**~~ | **RETIRED 2026-09-17, and the property it proved is now UNPROVEN** — see `docs/STATIC-EXPORT-MIGRATION.md`. The 2026-09-16 measurement below was real, and it measured a mechanism that no longer exists. **How it was measured, when it existed**: scenario 11 gave a before/after pair — after advancing the index, the same page request returned `Indexed to block` **12639 → 12648**, `Lag` **97 → 2293**, `indexer last ran` **1h 13m → 0s** (the recorded run; the values move with the chain, so the assertion is that each one CHANGED). `tools/capture-scenarios.mjs` ran the whole sequence — capture, advance the index, capture again, judge the pair — so the proof was one command rather than a procedure | The mechanism is gone: the pages are client components in a static export, so there is no `force-dynamic` to take effect and no server to re-read. `docs/evidence/scenario-11-freshness.txt` is kept as the record of what was measured, and it is **no longer evidence for the shipped artifact**. Its replacement is `staleTime: 0` + `refetchOnWindowFocus` in `src/app/providers.tsx`, and the honest way to re-measure it is the row-65 procedure: load the published page twice, capture the RPC requests each load issues, record the pair | **Re-measure before relying on it** |
...
> **One of those three resolutions did not survive the static export**: `force-dynamic` taking
> effect is retired as of 2026-09-17, because the pages are client components in a static export
> and the mechanism it measured is gone — see `docs/STATIC-EXPORT-MIGRATION.md`, and the same
> correction in §1's row for it. Build reproducibility and the two-sources independence are
