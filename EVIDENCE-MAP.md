...
| **the zero-range guard** (a flat series must still be drawable) | **Pure function tests** (property assertions) + **real browser assertions on coordinate attributes** | `test/chart-geometry.test.ts` → `expands a flat range instead of dividing by zero` and 3 others, 4 in total<br>`tools/browser-assert.mjs` → `NO NaN coordinates` (172 lines / 169 rectangles / **0 NaN**) | the scale function produces finite coordinates **for all inputs**; and the browser **really did** draw those coordinates | the pure function layer **cannot** prove the browser drew anything (NaN is silent in SVG). The browser layer **cannot** prove other series shapes are safe (it only sees this one flat dataset). Neither layer is sufficient alone — which is exactly why both are kept |
...
| **the two sources fail independently** | **Structural assertions + the real service** (partly) | `src/app/page.tsx`'s `Promise.allSettled` + four independent branches<br>`test/api.test.ts` → 6 failure-classification assertions<br>`tools/browser-assert.mjs` → `the four panels rendered`, `no service-failure panel is showing` | the classification logic is right; when both are available all four panels are there and no failure copy is shown | **cannot** prove "after one service is stopped the other panel is still there" — because both services were alive when the assertion ran. The three stop-the-service re-runs it needs are in `BROWSER-TEST-PLAN.md` §5, status **not run**. This is the project's largest evidence gap and it is **recorded** |
...
| **`dynamic = 'force-dynamic'` takes effect** | **No automated assertion** | `src/app/page.tsx` lines 28–29 | — | **cannot** prove anything. This row is **currently only a claim in the source code**. The check available is: two requests return different content (`lagBlocks` or a timestamp changing), and this project **did not** write that as an assertion. Recorded as an open item §4 |
...
| **after one service is stopped the other panel is still there** (INV-03's core) | no double can simulate "a real process dying" | **Measured.** Injection method: start a console instance whose `VAULT_RPC` / `VAULT_API` point at **a port nothing listens on** — from the code under test's point of view that is exactly equivalent to "the process died" (connection refused), and the three shared services were never interrupted once. Scenario 9 (RPC down) `Now` errors while `Then` still renders 174 candles; scenario 10 (API down) the reverse | **Verified** 2026-09-16 (`docs/evidence/scenario-9-chain-down.txt`, `scenario-10-index-down.txt`) |
...
| ~~**`dynamic = 'force-dynamic'` takes effect**~~ | **Resolved, 2026-09-16.** Scenario 11 proved it with **a before/after pair**: after advancing the index, the same page request returned `Indexed to block` 12580 → **12639**, `Lag` 0 → **97**, `indexer last ran` 5m 11s → **4s**. Three values changing at once can only come from re-fetching | Done | — |
...
> The remaining **four** still have no evidence, each with its reason and plan written out.
>
...
- [x] Every "functionality" has content in both the "who can prove it" and the "what it cannot prove" columns (14 rows, no empty cells)
...
- [x] Every invariant in the ledger can find the evidence source it depends on in this file (INV-01…INV-08 mapped one by one)

**G-F2 conclusion**: **passing (self-assessed)** — 2026-09-16, the author.
14 functionality rows have their evidence complete, 7 "what it cannot prove" statements written out
one by one; §3 honestly lists 6 functionalities **with no evidence yet**, one of which is directly
relevant to a core design claim.
