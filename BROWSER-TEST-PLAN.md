...
> **Fifth amendment, 2026-09-19. The stub wallet was extended to every failure class a wallet's
> EIP-1193 surface can produce WITHOUT a signature, and it found two more defects in the same seam a
> person found the first two in. This block is added, not substituted: rows 1–8 above keep their
> status cells exactly as they stand, and what changes is the REASON each open one is open.**
>
> **What was done, and in what order.** First the failure classes the code promises and the interface
> renders were ENUMERATED against the copy that exists (`src/lib/txState.ts`, `src/lib/vaultActions.ts`),
> and the ones with no dedicated copy, no assertion or no evidence were written down. Then each class
> was injected through `tools/wallet-double-assert.mjs` — a stub EIP-1193 provider installed before any
> page script — into the **published** console, and what the page rendered was transcribed before
> anything was changed. **Then** the code was fixed where a class had no copy, and the same tool was
...
>   consequence (nothing was sent) nor the next action (reconnect, to which chain). Row 6's shape
>   exactly, one class over.
> - **A FAILED CHAIN SWITCH WAS NOT SAID AT ALL.** `wallet_switchEthereumChain` was called and
>   refused; the page re-rendered the SAME "Switch the wallet to chain 84532 -- it is currently on
>   chain 8453 …" refusal it had shown before the click, and said nothing about the attempt. Measured
>   with `4902` (MetaMask's own `Unrecognized chain ID "0x14a34". Try adding the chain using
...
> renders now.
>
> | Class (EIP-1193) | What the page rendered BEFORE | What it renders AFTER |
> |---|---|---|
...
> that chain does not simply forward the wallet's answer: driven in **Node** against viem 2.56.5 the
> refusal arrives at `mapWriteError`, but driven **in the browser** through this stub the same refusal
> is answered by the app's own RPC instead — the app's transport log shows `eth_call`s where the
> wallet's error was expected, and no wallet error is ever the one the page reports. A synthetic
> receipt was also attempted (serving `eth_getTransactionReceipt` for a synthetic hash through the
> app's own transport, and a `status: 0x0` body) and did not reach `mapReceipt` either. So:
> **the stub covers every class the wallet's EIP-1193 surface produces on its own, and none of the
> classes that need a chain.** Those are proven where they can be — `test/wallet-errors.test.ts`
> asserts the classification against real viem error shapes, including both of the two that share
> code `-32000` and differ only by class — and their browser half stays with a person.
>
> **Which rows gained measured coverage, and which remain — the row-by-row accounting.**
>
> | §5 row | Before this amendment | After it | Why the remaining part needs a human |
> |---|---|---|---|
> | **1** wallet not installed | `implemented; interaction not measured` | **unchanged** | A stub PROVIDER's whole job is to be present, so no version of this double can produce "no provider". Needs a browser with the extension removed — a person, or a clean profile |
> | **2** refused prompt (`4001`) | partly measured; row 2's fix verified 21/21 | **measured for the error path** — the rejection is asserted end to end, and a rejection that mentions gas, or a chain, is still rendered as a cancellation | The prompt itself: that MetaMask shows one and that a person's Reject is what produces the `4001` |
> | **3** wrong chain | partly measured; guard fix verified 21/21 | **unchanged, and unchanged on purpose** — the settled switch and the same-task race are both asserted (B, C1, C2) | A real wallet switch, and that MetaMask names these two chains |
> | **4** insufficient allowance | partly measured by a post-hoc capture | **unchanged** | The approve offered instead of a deposit is measured; that the approve is *signed* and the allowance *re-read* needs a real signature |
> | **5** insufficient balance | logic proven | **unchanged** | Same: the refusal is a pre-flight decision, asserted; driving it through a wallet changes nothing about it |
> | **6** insufficient gas | **no copy at all** (a recorded gap) | **the copy now exists and the class is asserted** (`test/wallet-errors.test.ts`), **and the browser half is still open** | A wallet that cannot pay for gas, and a chain that refuses for funds — the stub cannot make either reach the page (measured above) |
> | **7** reverted | logic proven | **unchanged** | A chain that reverts a real transaction. The receipt path is asserted in unit tests and the synthetic-receipt attempt is recorded above |
> | **8** replaced | no dedicated state; recorded gap | **unchanged** | A wallet that replaces a pending transaction. The unread-receipt sentence is what covers it, asserted in `mapReceipt`'s tests |
> | **9, 10, 11** the read paths | passed 2026-09-16 | **unchanged** | Nothing: these three are passed by measurement and were not touched |
>
> **HOW MANY ROWS STILL NEED A HUMAN, AND WHY — the number this amendment exists to make small.**
> **Eight of §5's eleven rows still have something only a person can produce** (rows 1–8; rows 9–11
> are passed by measurement and need nobody), and the honest count of what remains is:
>
> - **1 row** needs a browser with no wallet extension at all (row 1);
> - **7 rows** need a real MetaMask prompt, signature or chain — rows 2–8, and of those, **rows 4 and
>   5 are pre-flight decisions that are already proven as far as a program can prove them**: what
>   remains for them is the same prompt and signature as the rest, not a class of their own.
>
> What changed is the SIZE of that human half: **it was "the whole interaction" for eight rows, and it
> is now four steps** (`TEST-DOUBLES.md` §2a), because each act that could be automated was removed
> from the list rather than left on it. The four steps, and the one thing each needs that no program
> has:
>
> 1. reject an approve prompt — a person's decision at a prompt;
> 2. a chain switch refused at the prompt — the same, on the switch path;
...
> **Sixth amendment, 2026-09-18 (local clock `2026-09-18 10:5x–13:5x +08:00`). All four steps of
> `TEST-DOUBLES.md` §2a were driven by a person and measured. One of them could not be run as it was
> written — and that is the substance of this amendment, because "the step was hard" and "the step was
> impossible" lead to different documents.** The measurements are in
> `verification/out/manual-wallet-2026-09-18.txt`; the captures are named below.
>
> **1. Row 2's step could not produce a prompt, and the wallet's own permission store says why.**
> The step asks a person to put MetaMask on Base mainnet, click the app's `Switch to chain 84532`, and
> reject the window. The window does not come. Read from the wallet's EIP-2255 store, approving
> **CONNECT** grants the site `endowment:permitted-chains` covering **every network the wallet holds**
> — eleven of them, `0x14a34` among them — and a site that already holds a chain is switched to it
> **silently**. Two origins were driven before this was understood: on the first the grant read
> `["0x2105"]` after a *switch* approval, on the second it read all eleven after a *connect* approval,
> and both switched with no window. The executable form is the reverse order: make a **switch request**,
> not a connect, the origin's first approval. That grants exactly the chain it names, which leaves
> `84532` as the one chain that still needs a prompt.
>
> **2. Driven that way, the refusal is measured, not inferred.** With `window.ethereum.request`
> instrumented on the page so the traffic itself is on record: the app's own control sent
> `wallet_switchEthereumChain {"chainId":"0x14a34"}`; real MetaMask answered **`code 4001,
> "User rejected the request."`**; the page rendered *"The chain switch was cancelled in the wallet, so
> the wallet is still not on chain 84532 (Base Sepolia). Nothing is sent until it is."*; the wallet
> stayed on its chain; `84532` was **not** added to the site's permitted list; the switch control was
> still offered and **no write control was**; and the account's nonce did not move (`6 → 6`, block
> 46,966,449). The same hour produced a real **`4902`** on an origin where the wallet no longer held
> Base Sepolia, rendered as the `chain-not-added` sentence — a second error class measured against the
> real wallet rather than against the stub.
>
> **3. Row 4 and §6's step 3, on the PUBLISHED site, from a zero allowance to a mined deposit.**
> approve `0x71b0dfb13ea866d9821c54e0b4e25c582b160f504762cb77d661e866976606e8`, block **46,971,112**:
> USDC `Approval(owner = 0x2aE7…E034, spender = 0x7941438e…7f35 the vault, value = 1000000)`.
> deposit `0x8f114b1d30d1373cfab3d0fd2ce35c78221d165b60bce0bf88dad0f784a24551`, block **46,971,145**,
> `type 0x2`, nonce 6, from the account to the vault, `deposit(1000000, 0x2aE7…E034)`, whose logs are
> the USDC transfer, the vault's share mint, and
> `Deposit(sender = owner = 0x2aE7…E034, assets = 1000000, shares = 1e18)`. Before → after, read from
> the chain: nonce `6 → 7`; the account's USDC `18 → 17`; its shares `22 → 23`; the allowance
> `0 → 1000000 → 0`; the vault `22 → 23`. The page re-read after a reload — fresh reads, not the
> post-confirmation render — shows `17 USDC`, `23 shares`, `allowance 0`, and no failure text. The
> allowance's **re-read across two successive reads** is visible in the session itself: after the
> approval the form said *"The allowance already covers this deposit, so only the deposit transaction
> is needed."* and offered the deposit alone, which is the state that only a fresh read can produce.
>
> **The relay question moved rather than closed.** This time it was the **approve** that was executed
> on the account's behalf — `from 0xb01caea8c6c47bbf4f4b4c5080ca642043359c2e`, `to
> 0xdb9b1e94b5b69df7e401ddbede43491141047db3` (the MetaMask `DelegationManager`), with the encoded
> `approve` call (selector `095ea7b3`, spender the vault, amount `1000000`) carried inside the
> delegation framework's own event — while the **deposit** was sent directly by the account. The
> 2026-09-17 session had it the other way round: its deposit went through the same manager and its
> approve did not. Both routes are now identified **for both writes**, and what chooses between them is
> still not explained. It is recorded as open rather than guessed at.
>
> **4. The capture gap §8 records is closed, and it is the frame this plan said was missing.**
> `verification/out/screen-metamask-prompt.png` is a full screen (3837x2397) showing the published page
> with the amount `1`, `vault allowance 0 USDC (read from the chain, never remembered)`, the control in
> its **pending** state reading `Waiting for the wallet…`, and in the foreground MetaMask's own
> spending-cap prompt: 1 USDC on `0x036CbD…`, spender `0x79414…37f35`, network Base Sepolia,
> "请求来自 hareeshkashyap849.github.io", with its cancel and confirm buttons. A real prompt and the
> page's pending phase in one image, at the moment of the decision rather than after it. The browser's
> instrumentation banner is visible in the same frame and was **not** cropped: the instrument is
> disclosed in the evidence rather than hidden from it.
>
> **5. Row 5 was measured with no help from a person at all, on the published page with the real
> connected wallet**: typing `999` against a balance of `17 USDC` renders *"That is more USDC than the
> wallet holds: 999 requested, 17 available."* and offers **no control at all** — not even the approve,
> so the refusal happens before anything could be signed and the number is the real balance rather than
> the bare word "insufficient". The field was cleared afterwards and the page re-read to confirm it.
>
> **6. The write is visible through the public system.** The published console's own snapshot now
...
> being clicked** — the two captures are the pending state and the state after the refusal, and the
> frame that shows the click itself is the one thing no instrument here can take. It also does not
> establish that a real wallet's **write** refusal reaches the page as `4001`: that is inferred from the
> page's rendered panel, which is the `rejected` branch and is reached by `4001` alone
> (`src/lib/walletError.ts` classifies `4001` before every other branch), and the code itself was
> recorded on the switch path and not on that one.
...
| 2 | The user refuses to sign (`4001`) | Reject at the MetaMask prompt | A **neutral** line, never a red failure; the form returns to idle and says the user cancelled and nothing was signed. `mapWriteError` checks `4001` first so no later branch can reclassify it | screenshot: **not run** | **logic proven** (`test/wallet-flow.test.ts`: *a user rejection is its own phase, not a failure*, and the nested-cause case); **interaction not measured** — **AND ONE REAL CLICK WAS ANSWERED WRONGLY, 2026-09-18**: a person pressed Reject and the published page rendered `Waiting for the chain(approve)` / `Approval sent. The wallet prompt is done; this waits for the chain to include it.` while the chain reported `allowance 0` and an unmoved nonce. The taxonomy was right and the render overwrote it; fixed in `413ce8e` (`txStateFor`), re-measured on the published page at **21/21** with a stub provider that answers `4001`. **Still not `passed`**: no real wallet prompt was driven, so "MetaMask's prompt is followed by this wording in a real session" remains unmeasured — what is measured is the page's answer to a rejection. **SUPERSEDED IN PART, SIXTH AMENDMENT (2026-09-18): the prompt was driven, and the page answered it as this row says it should.** A person rejected the approval prompt on the published page; the page rendered its cancellation panel — `You cancelled this(approve)` with *"You cancelled this in the wallet, so nothing was signed and nothing was sent. The form is back where it was; no gas was spent."* — the allowance still read `0` and the account's nonce did not move. What is still not on record for this path is the **code**: the `4001` was captured on the switch path (sixth amendment, item 2) and on this one the panel is what was read, and the panel is the branch `4001` alone reaches (`src/lib/walletError.ts` tests `4001` before every other code) |
| 3 | Wrong chain | Wallet on chain 8453, page expecting 31337 | The controls are **disabled with a reason naming both chains** — *"Switch the wallet to chain 31337 — it is currently on chain 8453, where this deployment does not exist. Nothing is sent until it does."* — and **no wallet prompt appears** | screenshot: **not run** + transaction hash: **not applicable, nothing is sent** | **logic proven** (`decideDeposit` → `wrong-chain`, and it outranks an unparseable amount); **interaction not measured** — **AND THE GUARD COULD NOT FIRE, 2026-09-18**: the injected chain was 8453 against a deployment on **84532** (the `31337` above is the pre-Base-Sepolia copy), and the published page read *"Wallet chain 84532 / matches the deployment"* while the wallet was on 8453, offered `1. Approve USDC`, and put an `approve` in front of the wallet on that foreign chain — the failure came back from the chain (`gas required exceeds allowance (0)` from `0x2105`), not from the app. The guard's input was `useChainId()`, which is the app's own chain and is not updated for a chain outside `config.chains`, so the check compared one value with itself. Fixed in `413ce8e` (the connection's chain, plus the wallet asked again at the moment of each send); the published page now names both chains and offers no control, measured at **21/21**. **Still not `passed`**: no real wallet switch was driven, so the row's "no wallet prompt appears" half rests on the stub rather than on MetaMask. **SUPERSEDED IN PART, SIXTH AMENDMENT (2026-09-18): both halves were driven with a real wallet.** With the connected wallet on Base mainnet and the deployment on 84532, the page rendered the disagreement in this row's shape — a notice naming both chains (*"The wallet is on chain 8453 and this deployment is on chain 84532, so the figures here would come from a different chain. Nothing is sent until they agree."*), **no write control anywhere on the page**, and the switch control as the only offer. And the switch it offered was refused by a person, with MetaMask answering `4001` and `84532` absent from the site's permitted list afterwards; a real `4902` on the same path rendered the `chain-not-added` sentence. The measurement ran against the same build served from `out/` on a scratch origin, which the sixth amendment states as a condition rather than leaving it to be discovered |
| 4 | Insufficient allowance | Fresh wallet, zero allowance, then deposit | The **approve step is offered instead of a deposit**; a deposit that would revert with `ERC20InsufficientAllowance` is never sent | screenshot: `verification/out/wallet-manage-approve-step-offered-2026-09-18.png` — **post-hoc; it does not show the signing** (third amendment) | **logic proven**, including the case where the allowance was consumed and must be re-read; **partly measured 2026-09-17** — a person deposited from a zero allowance and the chain now reports **`1000000`** (1.0 USDC) on the account named in the amendment above, so the approve really did run and change state. **The approving transaction is now identified**: `0xac558a4be8b234374e64a6be08fc9532fe2488f28dbc46cf495cfeeb7dd00ffc`, block **46945057**, `Approval` value **`1000000`**, spender the vault, `from` the account itself (see the second amendment). **The page's offer is now measured too** (third amendment): with the allowance at `0` and an amount typed in, the published form rendered *"The vault's allowance is 0 USDC, which does not cover this deposit. Approving is the next step; the deposit follows it."* and offered **`1. Approve USDC`** — so the row's central claim, *the approve step is offered instead of a deposit*, is read off the page rather than inferred. **Still not `passed`**: the half of the row about a reverting deposit never being sent is proven only in a unit test, and the capture is after both transactions, so it shows the form's decision and not the interaction — **and on 2026-09-18 this row's control was clicked for real, twice over: a person pressed `Deposit`, the wallet showed the approve prompt for exactly this `1. Approve USDC`, and the two clicks at that prompt (Reject, and a send from the wrong chain) are the two defects recorded in the fourth amendment, both fixed in `413ce8e`. The fourth amendment also re-measured this offer on the published page after the fix (`1. Approve USDC` for 1 unit against `allowance 0`), so the offer itself still holds**. **SUPERSEDED IN PART, SIXTH AMENDMENT (2026-09-18): the offer, the signature and the re-read were driven end to end on the published site.** With the allowance at `0` and `1` typed in, the form offered `1. Approve USDC`; a person approved; the page then said *"Approval confirmed. The allowance is re-read from the chain before the deposit is offered."* and offered the **deposit alone**, under *"The allowance already covers this deposit, so only the deposit transaction is needed."* — the re-read in the state rather than in the wording, because only a fresh read can produce it. The approval is `0x71b0dfb1…` (block 46,971,112, USDC `Approval` = `1000000`, spender the vault) and the deposit `0x8f114b1d…` (block 46,971,145), which consumed the allowance back to `0` and moved the vault and the account to `23`. What this row still does not have is the **capture of the offer being signed** — the frame that exists is the prompt beside the pending form — and the reverting-deposit half is still a unit test |
| 5 | Insufficient balance | Enter more than the wallet holds | The reason carries the **real balance, formatted** (`5555.0759` and the symbol), never the bare word "insufficient"; refused **before** any approval, because an approval needs no balance and approving first would spend gas to learn a free fact | screenshot: **not run** | **logic proven** (`decideDeposit` → `exceeds-balance`, and the balance check is asserted to win over the allowance check); **interaction not measured** — **MEASURED ON THE PUBLISHED PAGE, SIXTH AMENDMENT (2026-09-18): typing `999` against a connected wallet holding `17 USDC` rendered *"That is more USDC than the wallet holds: 999 requested, 17 available."* and offered no control at all — not even the approve.** So the refusal is read off the page with the real wallet connected, the number in it is the real balance rather than the bare word "insufficient", and the "refused before any approval" half is measured rather than inferred: there is no approval control to click. The field was cleared and the page re-read afterwards; the screenshot cell is still `not run` because the capture was not taken while the refusal was on screen |
...
- [x] every failure case **that has been run** has a screenshot — `scenario-9-chain-down.png`,
      ...
      listed here as screenshots of the interface, not as evidence of the moments the rows describe. **Sixth amendment (2026-09-18): a capture of the moment itself now exists for the write path — `verification/out/screen-metamask-prompt.png`, a full screen showing MetaMask's spending-cap prompt with the page's own `Waiting for the wallet…` pending state behind it — so the limitation this bullet is about is no longer that every wallet capture is post-hoc. It is that the rejection click itself was not photographed, which no tool here can do.**
...
**Fourth amendment to this verdict, 2026-09-18 (local clock `2026-09-18 13:5x +08:00`): the verdict does
not move, and the set of rows holding it open is now four — each with a named instrument in its way
rather than the word "wallet".** The measurements are §5's sixth amendment and
`verification/out/manual-wallet-2026-09-18.txt`.

| Row | State after this session | What still blocks it |
|---|---|---|
| **1** wallet not installed | untouched | a browser with **no** extension. The instrument here is the user's real browser with MetaMask in it, so this row cannot be produced without a second, empty browser profile |
| **2** refused signature | **measured**: a person rejected the prompt on the published page, the page rendered its cancellation panel, the allowance stayed `0` and the nonce did not move | the wallet's own `4001` was captured on the **switch** path, not on this one. A capture of the prompt being rejected is not possible with the tools here |
| **3** wrong chain | **measured**: with a real wallet on Base mainnet and the deployment on 84532, the page named both chains and offered **no** write control; the switch control it did offer was refused, MetaMask answering `4001` | the measurement ran against the same build at a scratch origin, stated rather than hidden — the published origin cannot be put back into that state once it holds a permitted chain |
| **4** insufficient allowance | **measured**: offer → signature → receipt → the allowance re-read as covering the deposit, and consumed back to `0` by the mined deposit | the capture of the offer being signed, and the reverting-deposit half, which the form refuses to send and a unit test covers |
| **5** insufficient balance | **measured**, with no help from a person: `999` against `17 USDC` renders the refusal with the real balance and offers **no control at all** | a screenshot taken while the refusal is on screen — the field was cleared afterwards |
| **6** insufficient gas | capture obtained, class still unproduced | a real under-funded wallet. Measured why the stub cannot stand in: the app's own RPC answers the call before the wallet does (fifth amendment) |
| **7** reverted | untouched | a revert the page will actually send. The pre-flight refuses exactly the states that would produce one, which is the row's own feature working against its measurement |
| **8** replaced | untouched | a transaction replaced in the wallet's own UI — a person, and a wallet willing to be raced |

**Rows 9–11** are unchanged and passed by measurement; **§6's step 3** (deposit) now has a hash, a
block, a decoded event and the state change either side of it, and **step 4** (redeem) is still
untouched and still the one write path never driven.

**§7's console gap is also unchanged**: console events are still not captured directly, and the
substitute remains "assert the page text carries no error and no hydration hint". Nothing in this
session's measurements narrows that, and it is listed here so the four-row count above is not read as
...
>
> **Second addendum, 2026-09-18 (later the same day): steps 2 and 3 have both hashes, both receipts, the
> decoded `Deposit` event, and the state change either side of them — and the frame that was missing.**
> `verification/out/screen-metamask-prompt.png` shows MetaMask's own spending-cap prompt with this page's
> `Waiting for the wallet…` pending state behind it, taken at the moment of the decision rather than after
> it, so step 3's gap is no longer "no capture of the interaction". What remains is narrower and is named
> in §5's sixth amendment: **the approval was executed on the account's behalf through the EIP-7702
> delegation path while the deposit was sent directly by the account, and nothing here explains what
> chooses between the two routes** — the 2026-09-17 session had them the other way round. Step 4 (redeem)
> is still untouched and still the only write path never driven.
...
- The export **serves every route**; a real browser renders all four, and each **settles** out of the loading
  screen (measured: 3,433 / 2,285 / 2,138 / 2,406 characters, the console settling on its 6th and its 8th poll
  in two runs -- the poll count moves, which is why the wait is a property of the page rather than a sleep).
