...
| No JavaScript | **unsupported, and more strongly so than before**: every route is a client component in a static export. Without JS the reader gets the `Providers` loading screen (`Loading the deployment record…`) and nothing else — the addresses are read at runtime from `api/config`, so there is no server-rendered panel text to fall back on. Under the previous server components this row was milder (the panels' text was in the HTML, and only the chart needed hydration); `docs/STATIC-EXPORT-MIGRATION.md` records the change |
...
| Deployment record missing or unusable | `<configPath> could not be fetched: <cause>` and `<configPath> answered <status>. It is generated by scripts/build-runtime-config.mjs.` — and at build time, `no deployment record at <path>` plus the command that writes one | ✅ both halves name **which file** was wrong. The reader's half names the generator, so the fix is one step away instead of "the page is broken"; the generator's half refuses to write a config at all rather than writing one with no address, which is what makes `Providers` safe to stop the app on |
| A field is missing from the deployment record | `<path> is not usable as a runtime config:\n  - vault is not an address: <value>` (also `asset`, `chainId`, `deployBlock`, `rpcUrl`, `indexApiUrl`), and at build time `${recordPath} is not usable as a deployment record:` with the same per-field list | ✅ it says which field and what the value was, and it **refuses to guess** — the generator writes nothing and the reader throws before any page renders, which is exactly why this error exists |
...
| ~~6~~ | ~~**`dynamic = 'force-dynamic'` takes effect**~~ | **retired 2026-09-17, not resolved**: the 2026-09-16 scenario-11 measurement was real and proved the *server* re-read per request (`Indexed to block` 12580→12639, `Lag` 0→97, `last ran` 5m11s→4s, `docs/evidence/scenario-11-freshness.txt`). The console is a **static export** now: `page.tsx` no longer exports `dynamic = 'force-dynamic'`, there is no server to re-read, and that measurement cannot cover the published pages. The replacement — `staleTime: 0` + `refetchOnWindowFocus` in `src/app/providers.tsx`, plus `test/api.test.ts`'s `no-store` assertion — is **a unit test and a structural argument, not a measurement**, so this item returns to "outstanding" rather than staying resolved. Re-measure by loading the published page twice and capturing the RPC requests each load issues. `docs/STATIC-EXPORT-MIGRATION.md` is the authoritative record | — | — |
