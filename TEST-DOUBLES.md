...
| **a "chain not added" error takes the add-chain route** (added 2026-09-18) | **the double, and one real observation that disagrees with it — this row is the double being caught.** `src/lib/walletFailureCopy.ts` recorded, measured against the stub, that wagmi's `switchChain` OFFERS the chain on `4902` and that the error reaching the app is the ADD's own `4001`. On the real wallet the app's own switch control rendered the `chain-not-added` sentence **with no window raised at all**, and the shipped `@wagmi/core` 3.7.7 (`connectors/injected.js`) enters that branch only when `error.code === 4902` or `error.data.originalError.code === 4902`, wrapping every exit from it as `UserRejectedRequestError` (`4001`) — which `classifyWalletError` answers as `rejected`, never as `chain-not-added` | **No, and the double is the thing that is wrong here.** The stub's error carries exactly the code the connector tests, so the stub always takes the branch the real wallet skipped. The mechanism behind the real render is **not settled**: either no level carried a numeric `4902` and the classifier matched the WORDING, or a `4902` sat at a depth the connector's check does not read while `4001` appeared nowhere. One instrumented run against a wallet that does not hold the chain would say which |
