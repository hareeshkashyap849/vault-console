...
The scenario is kept as a **guard**; the code change it produced is kept as **defensive**, because the
shape it guards (an address reconstructed by wagmi from its own store, with the wallet no longer
offering one) is one wagmi's store permits; and **no defect is claimed for it anywhere.**
