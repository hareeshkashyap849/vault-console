...
| the stub EIP-1193 provider — **prompt, signature, relay** | **Cannot be done**: a real prompt needs a person and a funded key, and this repository's rule is that the work sends no transaction. **Manual check steps for a person**: open `/vault/manage` with a real wallet on Base Sepolia, enter an amount, and (a) reject the prompt — the page must say *"You cancelled this"* and leave the control usable; (b) switch the wallet to another network and re-enter the amount — the page must name both chains and offer no control. The stub asserts the same two outcomes, which is why they are written down here as a repeatable check rather than as a claim | the author | **Partly unverified** — the rendered wording is measured; the real prompt is not |
